...
class PolymerDataset(InMemoryDataset):
    ...
    def raw_dir(self) -> str:
        return str(Path(self.root).resolve().parent / "data")
    ...
    def process(self):
        ...
        default_source = Path(self.root).resolve().parent / "data" / "ES.csv"
        override = os.environ.get("BBMODELS_DATA_CSV")
        if override:
            override_path = Path(override).expanduser()
            if not override_path.is_absolute():
                override_path = (default_source.parent.parent / override_path).resolve()
            source = override_path
        else:
            source = default_source
        if not source.exists():
            raise FileNotFoundError(f"Expected dataset at {source}")
