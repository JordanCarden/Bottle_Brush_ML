...
DEFAULT_DATASET = Path("test_set.csv")
DEFAULT_SUMMARY = Path("test_performance.csv")
PREDICT_SCRIPTS: Tuple[Tuple[str, Path], ...] = (
    ("CNN", Path("CNN") / "predict.py"),
    ("GAT", Path("GAT") / "predict.py"),
    ("GCN", Path("GCN") / "predict.py"),
    ("GIN", Path("GIN") / "predict.py"),
    ("LR", Path("LR") / "predict.py"),
    ("MLP", Path("MLP") / "predict.py"),
)
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    prediction_files = _find_prediction_files(Path("."))
