...
class GAT(nn.Module):
    """Three-layer graph attention network with global mean pooling."""

    def __init__(self, in_dim: int, hidden_dim: int, heads: int, dropout: float, attn_dropout: float) -> None:
        super().__init__()
        self.conv1 = GATConv(in_dim, hidden_dim, heads=heads, concat=False, dropout=attn_dropout)
        self.conv2 = GATConv(hidden_dim, hidden_dim, heads=heads, concat=False, dropout=attn_dropout)
        self.conv3 = GATConv(hidden_dim, hidden_dim, heads=heads, concat=False, dropout=attn_dropout)
...
def evaluate(
    ...
    metrics = compute_regression_metrics(gt_arr, pred_arr)
    return avg_loss, metrics, pred_arr, gt_arr
    ...
    for fold, (train_idx, val_idx) in enumerate(splits, 1):
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            val_loss, val_metrics, val_preds, val_targets = evaluate(
                model, val_loader, mean, std, device, target_id
            )
            ...
            if should_log:
                train_rmse = float(np.sqrt(train_loss)) * float(std.item())
                print(
    ...
    try:
        for idx, (lr, wd) in enumerate(combos, 1):
            ...
            epochs, summary, predictions = cross_validate_epochs(
                dataset, target_id, args, device, splits
            )
            record = {
                "trial": idx,
                "lr": float(lr),
...
def train_full_dataset(
    ...
    args: argparse.Namespace,
    device: torch.device,
) -> Tuple[GAT, Dict[str, float], Dict[str, float]]:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train GAT models with K-fold epoch selection"
    )
    parser.add_argument(
        "--data",
    ...
    parser.add_argument(
        "--weight_decay", type=float, default=1e-5, help="Weight decay for Adam optimizer."
    )
    parser.add_argument(
        "--patience",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Path to a JSON file containing K-fold train/val indices.",
    )
    parser.add_argument(
        "--hidden_dim", type=int, default=128, help="Hidden dimension size."
    )
    parser.add_argument(
        "--heads", type=int, default=4, help="Number of attention heads per GAT layer."
    )
    parser.add_argument(
        "--dropout",
        type=float,
        default=0.2,
        help="Dropout applied after attention layers and the penultimate linear layer.",
    )
    parser.add_argument(
        "--attn_dropout",
        type=float,
        default=0.2,
        help="Edge attention dropout used inside GATConv layers.",
    )
    parser.add_argument(
        "--skip_cv",
        action="store_true",
