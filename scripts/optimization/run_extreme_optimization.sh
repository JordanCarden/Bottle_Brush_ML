...
EXTREME_FILE="${SCRIPT_DIR}/extreme_test_set.txt"
TOP_CANDIDATES_FILE="${LOG_DIR}/top_candidates_per_run.txt"
...
for LENGTH in "${LENGTHS[@]}"; do
    for COMBO in "${COMBOS[@]}"; do
        ...
        python3 "$SCRIPT_DIR/optimization.py" \
            --method "$METHOD" \
            --length "$LENGTH" \
            --use-area --area-mode "$AREA_MODE" \
            --use-rg --rg-mode "$RG_MODE" \
            --use-rdf --rdf-mode "$RDF_MODE" \
            2>&1 | tee "${LOG_DIR}/extreme_run${RUN_NUM}_len${LENGTH}_A${AREA_MODE}_R${RG_MODE}_D${RDF_MODE}.log"

        # Save the CSV file with a unique name for this run (before it gets overwritten)
        if [ -f "${LOG_DIR}/flex_len${LENGTH}.csv" ]; then
            cp "${LOG_DIR}/flex_len${LENGTH}.csv" "${LOG_DIR}/run${RUN_NUM}_candidates.csv"
        fi
...
echo "Logs saved to ${LOG_DIR}/extreme_run*.log"
echo "=========================================="
echo

# Extract optimized polymers and create extreme test set
echo "Collecting results into ${EXTREME_FILE}..."
...
for i in {1..8}; do
    LOGFILE=$(find "$LOG_DIR" -maxdepth 1 -name "extreme_run${i}_*.log" | sort | head -n 1)
    if [ -f "$LOGFILE" ]; then
...
export LOG_DIR TOP_CANDIDATES_FILE
...
top_candidates_file = Path(os.environ["TOP_CANDIDATES_FILE"])

for run_num in range(1, 9):
    csv_file = log_dir / f"run{run_num}_candidates.csv"
...
# Write to file
with top_candidates_file.open('w') as f:
    f.write('\n'.join(output_lines))
