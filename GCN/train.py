...
def cross_validate_epochs(
    ...
    for fold, (train_idx, val_idx) in enumerate(splits, 1):
        ...
        model = GCN(in_dim=3, hidden_dim=args.hidden_dim, dropout=args.dropout).to(device)
...
def train_full_dataset(
    ...
    args: argparse.Namespace,
    device: torch.device,
) -> Tuple[GCN, Dict[str, float], Dict[str, float]]:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train GCN models with K-fold epoch selection"
    )
    parser.add_argument(
        "--data",
